#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""`P11.21-k` — UN AVEU SE PROUVE LÀ OÙ IL SE VOIT, PAS LÀ OÙ IL EST ÉCRIT.
...
CE QUI EST EN JEU. `web/attack.js` (`loadAttackMatrix`) rend, quand le démon sert une
matrice ET en nomme la cause, un AVEU — « couverture ATT&CK PARTIELLEMENT LUE … » —
qu'il pose AVANT la matrice, parce qu'un démenti rencontré APRÈS le tableau est
rencontré après que le lecteur a compté. Et `techniqueCell` colle un signe « au
moins » au SEUL nombre qui est un minorant. Les deux propriétés sont, à ce jour,
tenues UNIQUEMENT par le rang dans le document. Une règle de feuille de style qui
masquerait l'aveu, le rendrait transparent, le repousserait hors du document ou
INVERSERAIT sa position peinte les laisserait VERTES.
...
les polices, les chemins absolus et la chaîne d'ancêtres de production — y peint un
balisage DÉRIVÉ de `web/attack.js`, et demande à un moteur de rendu ce qui est
réellement peint. Cinq propriétés :

  1. L'aveu n'est ni masqué, ni transparent, ni SANS ENCRE, ni de rectangle vide, ni
     repoussé hors du document — chez lui OU chez n'importe lequel de ses ancêtres.
     « Sans encre » couvre le cas que ni `display`, ni `visibility`, ni `opacity`, ni la
     géométrie ne voient : `color:transparent`, ou une encre IDENTIQUE à son fond. Ce
     n'est PAS un critère de CONTRASTE — aucun rapport WCAG n'est imposé ici, et une
     encre simplement PÂLE passe.
  ...
  4. L'INSTRUMENT EST VALIDÉ DANS LES DEUX SENS, ET AVANT TOUT VERDICT : la même
  ...
  5. REFUS DE CONCLURE (code 2), JAMAIS UN VERT SILENCIEUX : aucun moteur, aucune
...
CE QUE CETTE GARDE NE TIENT PAS — écrit ici plutôt que découvert plus tard :
  · Le balisage peint est DÉRIVÉ de `web/attack.js`, pas produit en appelant
    `loadAttackMatrix` (qui exige un démon). Ce qui est jugé est donc la FEUILLE DE
    STYLE et la chaîne d'ancêtres, sur la forme que le module déclare émettre. Si le
    module cesse d'émettre l'une des marques dérivées, la garde REFUSE (code 2) au
  ...
  · L'ORDRE des deux nœuds est LU dans la source (`noeuds.push(aveu)` avant
    `noeuds.push(matrix)`) et rejoué tel quel : inverser la source fait rougir la
    propriété 2 ; mais la garde ne tient pas ce que fait `loadAttackMatrix` des
    autres chemins (refus, lecture entière).
  · Le panneau est RÉVÉLÉ ici comme `showView` le révèle (`section.hidden = false`,
    `html.app-ready`). Un défaut de la NAVIGATION qui n'ouvrirait jamais ce panneau
    n'est pas vu par cette garde.
  · Un seul gabarit de fenêtre est rendu (1600x1400). Une règle sous `@media` étroite
    qui masquerait l'aveu à une AUTRE largeur n'est pas vue.
  · AUCUN TEST DE RECOUVREMENT : un nœud opaque peint PAR-DESSUS l'aveu (`z-index`,
  ...
  · La garde juge l'aveu de la matrice ATT&CK. Les aveux jumeaux de `web/alerts.js`
    ne sont pas couverts par ce lot.
"""
...
def deriver_du_module() -> dict:
    ...
    j = src.find("function motDeLaMatriceIncomplete")
    if j < 0:
...
def main() -> int:
    ...
    aveugles = []
    for nom, regle, propriete in mutations(j):
        vus = juger(rendre(moteur, page, regle))
        touche = [m for m in vus if m.startswith(f"({propriete})")]
        if not touche:
            aveugles.append(f"« {nom} » (règle `{regle}`) attendue sur la propriété "
                            f"{propriete} — la page est rendue avec cette règle et la garde "
                            f"n'accuse rien ({len(vus)} manquement(s) : {vus})")
        else:
            print(f"[mutation vue] {nom} -> {touche[0][:150]}")
    if aveugles:
    ...
    if temoin_vert:
        print(f"::error::(1) {len(temoin_vert)} propriété(s) VIOLÉE(S) — l'aveu n'est pas "
              "peint là où il est lu :", file=sys.stderr)
        for m in temoin_vert:
            print(f"::error::  · {m}", file=sys.stderr)
        return CODE_VIOLATION

    print(f"[P11.21-k] VERT — {len(mutations(j))} mutations injectées ont TOUTES été vues "
          f"rouges (dont un masquage venu d'un ANCÊTRE en `:has()`/`@media`, et un ordre "
          f"PEINT inversé à rang de document INCHANGÉ) ; sans elles, `{moteur}` peint l'aveu "
          "visible, opaque, de rectangle non vide, dans le document, AU-DESSUS de la première "
          "cellule, et la marque de sous-compte n'est rognée par aucun débordement. "
          "CE QUE CE VERT NE DIT PAS : le balisage est DÉRIVÉ de `web/attack.js` et non produit "
          "par `loadAttackMatrix` (qui exige un démon) — ce qui est jugé est la FEUILLE DE STYLE "
          "et la chaîne d'ANCÊTRES sur la forme que le module déclare émettre ; un seul gabarit "
          "de fenêtre est rendu (1600x1400) ; il n'y a AUCUN test de recouvrement (un nœud "
          "peint par-dessus, un `clip-path`, ou une encre seulement PÂLE passent) ; et les aveux "
          "jumeaux de `web/alerts.js` ne sont pas couverts.")
    return CODE_OK
