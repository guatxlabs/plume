...
---------------------------------------
Le plan DÉCLARE son périmètre sur une ligne `Scope: \\`dir/\\`, … — except \\`nom\\`.` ; la garde
n'énumère aucun répertoire. Une SECTION est dans le périmètre quand son titre (`##`/`###`) cite,
...
Un nom nu (`exactness`) vaut `exactness.rs` ou `exactness/` ; `{a,b}` se développe ; `*` est un
motif sur l'arbre suivi ; un token finissant par `/` est un répertoire et doit contenir au moins un
fichier suivi. Un répertoire cité dans un titre est lui-même une revendication.
...
prose, et la prose n'est pas une entrée. Les deux jambes :
  ...
  (b) tout module de premier niveau d'un répertoire du périmètre — `nom.rs`, ou `nom/` ; `mod.rs`
      est le répertoire lui-même, pas un sous-module — a une entrée, sauf les noms exceptés.
...
# Relevé le 2026-08-22 : 69 entrées dans le périmètre, 226 fichiers suivis sous les répertoires déclarés.
MIN_ENTREES = 30
MIN_FICHIERS = 80
...
def lire_perimetre(texte: str) -> tuple[list[str], set[str]]:
    """La ligne `Scope:` du plan : répertoires déclarés (finissant par `/`) et noms exceptés."""
    ...
    dirs = [t for t in TOKEN.findall(part_dirs) if t.endswith("/")]
    exceptions = {t.strip("/") for t in TOKEN.findall(part_exc)}
    if not dirs or len(dirs) != len(TOKEN.findall(part_dirs)):
        raise InstrumentMuet("la ligne `Scope:` doit citer des répertoires entre accents graves, chacun "
                             "terminé par `/`.")
    return dirs, exceptions
...
def resoudre(token: str, bases: list[str], arbre: set[str]) -> set[str]:
    ...
    for forme in developper(token):
        ...
        for base in bases:
            ...
            if "*" in candidat:
                ...
                # Nom nu : un fichier `.rs` ou un répertoire.
                if candidat + ".rs" in arbre:
                    pour_forme = {candidat + ".rs"}
...
def relire(texte: str, arbre: set[str]) -> dict:
    ...
    dirs, exceptions = lire_perimetre(texte)
    ...
    for i, ligne in enumerate(lignes, 1):
        ...
        for t in tokens:
            bases = [b for b in [base_cellule, base, *dirs] if b]
            trouves = resoudre(t, bases, arbre)
            if not trouves:
                absents.append((i, t))
                continue
            revendiques |= trouves
    ...
    for d in dirs:
        ...
        for p in arbre:
            ...
            nom = reste.split("/", 1)[0]
            if "/" not in reste:
                if not nom.endswith(".rs") or nom == "mod.rs":
                    continue
                nom = nom[:-3]
...
def valider_instrument() -> list[str]:
    ...
    plan = "\n".join([
        "# Plan de contrôle",
        "Scope: `d/src/`, `d/src/c/` — except `tests`.",
    ...
    if not attendus <= r["revendiques"]:
        errs.append(f"témoin POSITIF en échec : non résolus {sorted(attendus - r['revendiques'])} — "
                    "une forme d'entrée que la garde DOIT reconnaître ne l'est plus.")
    if r["absents"] != [(14, "fantome.rs")]:
        errs.append(f"témoin en échec : chemins absents attendus [(14, 'fantome.rs')], obtenus {r['absents']} — "
                    "soit un chemin absent n'est plus nommé, soit une mention en prose / seconde cellule / "
                    "section hors périmètre est comptée comme une entrée.")
    if r["sans_chemin"] != [15]:
        errs.append(f"témoin en échec : lignes sans chemin attendues [15], obtenues {r['sans_chemin']}.")
    if r["modules_sans_entree"] != {"d/src/": ["e"], "d/src/c/": ["two"]}:
        errs.append(f"témoin en échec : modules sans entrée attendus {{d/src/: [e], d/src/c/: [two]}}, obtenus "
                    f"{r['modules_sans_entree']} — `tests` est excepté, `mod.rs` est le répertoire lui-même, "
                    "`h/` est couvert par son titre de section, `c/` aussi mais pas ses sous-modules.")
    if r["entrees"] != 5:
        errs.append(f"témoin en échec : 5 entrées attendues dans le périmètre, {r['entrees']} comptées.")
    try:
