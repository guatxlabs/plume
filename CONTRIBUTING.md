...
- **Mode-0 byte-identical.** Anything behind an OFF-by-default Cargo feature (or a runtime
  ...
  **same number** of tests before and after your gated change — **755** at the time of
...
(`SqlcipherStore` only) and must stay **755 tests green** and **offline**:

```sh
cargo test --manifest-path daemon/Cargo.toml        # 755 tests, default features
```
...
# 946 tests (= the 755 default tests + the cold_store tests the feature adds)
```
...
3. Include tests. Preserve or improve coverage; keep the default count at **755** (CI asserts it).
   If you touch `cold_store`, the `cold_tier` count (**946**) is asserted too.
