...
mod tests {
    ...
    fn drain_acks_all_and_saves_cursor() {
        ...
        assert_eq!(cursors.load("s1").valeur().cloned().flatten().as_deref(), Some("cur-1"), "curseur persisté APRÈS ack");
        assert_eq!(cursors.load("s2").valeur().cloned().flatten().as_deref(), Some("cur-2"));
