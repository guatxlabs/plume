  ...
  if [ ! -f "$RESP_ALLOW" ]; then
    {
      ...
      echo "# 1 ADRESSE par ligne."
      echo "#"
      echo "# CE QUI EPARGNE VRAIMENT : UNE EGALITE D OCTETS, PAS UNE ADRESSE."
      echo "# La recherche est \`grep -qxF\` : seule la ligne STRICTEMENT IDENTIQUE a la chaine que le"
      echo "# central envoie epargne. Ecrire 2001:0db8:0000:0000:0000:0000:0000:0001 n epargne donc PAS"
      echo "# 2001:db8::1, et 2001:DB8::1 n epargne pas 2001:db8::1 : ce lecteur les LIT toutes les"
      echo "# trois, il n en apparie aucune aux autres. Copiez la chaine telle que l alerte l affiche."
      echo "#"
      echo "# CE QUE CE LECTEUR REFUSE -- et un seul refus DESARME TOUT BAN SUR CET HOTE (fail-closed) :"
      echo "#   * un masque       203.0.113.0/24   2001:db8::/32"
      echo "#   * une zone        fe80::1%eth0"
      echo "#   * une IPv4-mappee ::ffff:203.0.113.7"
      echo "#   * un commentaire INDENTE, une ligne de BLANCS, un espace de fin, une fin de ligne CRLF"
      echo "# POURQUOI CE REFUS PLUTOT QU UN SILENCE : la recherche etant une egalite de ligne, une"
      echo "# forme que la cible ne porte pas n a JAMAIS epargne personne -- elle laissait le ban partir."
      echo "# Le refus est la meme absence de protection, mais DITE, et il ne touche pas unban_ip."
      echo "#"
      echo "# CE QUE CE LECTEUR ACCEPTE : l IPv4 pointee (203.0.113.7) et la forme hexadecimale a"
      echo "# deux-points (2001:db8::1). MESURE le 2026-08-28, ET IL FAUT LE LIRE : il accepte AUSSI des"
      echo "# chaines qui ne sont pas des adresses valides -- 999.999.999.999, 01.02.03.04, ab:cd, ::,"
      echo "# dead:beef, cafe: -- parce qu il teste une FORME, pas une adresse. Une telle ligne est donc"
      echo "# LISIBLE (elle ne desarme rien) et n epargne rien : c est un commentaire deguise. Ce fichier"
      echo "# ne dit pas ce qui est une adresse, il dit ce qui desarme et ce qui epargne."
      echo "#"
      echo "# CE QU IL NE SERT A RIEN D ECRIRE ICI, AUJOURD HUI :"
      echo "#   * une IPv6 -- ce produit ne sait PAS encore bannir une IPv6 (la borne d enforcement est"
      echo "#     IPv4, v1) : aucune action de ban IPv6 n est creee ni executee, donc une ligne IPv6"
      echo "#     n a jamais l occasion d epargner. Elle est LISIBLE, elle ne casse rien, elle ne"
      echo "#     protege rien. Ecrivez-la si vous voulez la garder prete ; ne comptez pas dessus."
      echo "#   * ::1, fe80::* , 127.* , 10.* , 172.16-31.* , 192.168.* , 169.254.* , fc00::/fd00:: ,"
      echo "#     et l hote central : DEJA exclus par ce script, les ecrire n ajoute rien."
      echo "# Ce qui a un effet ici : vos IPv4 PUBLIQUES d admin / sauts SSH de confiance."
      echo "#"
      echo "# N Y METTEZ PAS DE NOMS DE SERVICE : c est l autre politique (allowlist stop_service du"
      echo "# central, /etc/plume/responder.allow). Le demon REFUSE toute ligne qui a la FORME d une"
      echo "# adresse, masque et zone compris."
      echo "# CE QUI EST GARANTI ENTRE LES DEUX LECTEURS, ET RIEN DE PLUS : leurs deux criteres ne sont"
      echo "# PAS identiques. Le demon en reconnait strictement PLUS que ce lecteur-ci, si bien qu AUCUNE"
      echo "# ligne n est acceptee EN SILENCE par les deux -- mais une meme ligne peut etre refusee des"
      echo "# deux cotes. Corpus + balayage qui le mesurent sur les deux lecteurs :"
      echo "# collectors/predicat-adresse.corpus."
      echo "#"
      echo "# ET LE CENTRAL LE LIT AUSSI (P4.7-c, depuis le 2026-09-10) : le demon lit sa propre liste"
      echo "# d epargne, PLUME_CENTRAL_BAN_EXEMPT_FILE, dont le defaut est CE fichier. Il refuse un ban sur"
      echo "# une adresse ecrite ici en nommant l epargne ; une ligne illisible est ignoree et comptee au"
      echo "# registre never-ban SANS desarmer ses bans (cet hote-ci, lui, desarme) ; il ne relit le"
      echo "# fichier qu au redemarrage du demon."
    } > "$RESP_ALLOW"
