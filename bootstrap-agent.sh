  ...
  if [ ! -f "$RESP_ALLOW" ]; then
    {
      ...
      echo "#"
      echo "# ET CE QUE CE FICHIER NE PROTEGE PAS (P4.7-c) : le responder du CENTRAL ne lit AUCUNE liste"
      echo "# d epargne. Sur une machine a la fois centrale et agent, une action de ban reclamee par le"
      echo "# demon part sans passer par ici. Seules les plages reservees et l hote central sont"
      echo "# protegees de ce cote-la."
    } > "$RESP_ALLOW"
