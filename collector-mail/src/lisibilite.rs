...
//! AUX MÊMES MOTS. Ce collecteur ne partage aucune bibliothèque avec le démon ni avec l'agent — les
//! binaires sont autonomes par construction. Le vocabulaire, lui, est COMMUN : un test de ce crate lit
//! le module du démon et refuse toute dérive silencieuse de l'un des deux côtés.
//!
//! CE QUE CE MODULE FERME ICI, ET DANS QUEL ORDRE — ce collecteur ARME UNE DÉTECTION (IOC, phishing,
//! URL), donc chacun de ces silences rend une détection structurellement impossible :
//!   * un MESSAGE QU'ON NE SAIT PAS LIRE était marqué « vu » AVANT sa lecture, puis sauté : il n'était
//!     jamais analysé, il ne le serait plus JAMAIS, et le passage se concluait par « 0 alerte » ;
//!   * un MESSAGE QUE LE DÉCODEUR NE COMPREND PAS donnait des champs entièrement vides, qui ne
//!     déclenchent aucun motif : « rien à signaler » est alors la réponse à un message qu'on n'a pas
//!     su ouvrir — et c'est une réponse qu'un expéditeur peut PROVOQUER ;
//!   * un COMPTE ou un DOSSIER illisible était sauté en silence, pendant que le rapport comptait
//!     malgré tout ce compte parmi ceux qui ont été balayés ;
//!   * l'IDENTITÉ DE L'HÔTE rendait un nom plausible (`unknown`) quand sa source n'était pas lisible.
...
/// La source a été LUE mais sa forme n'est pas comprise. C'est le cas d'un message que le décodeur
/// MIME refuse : les octets sont là, ce qu'ils portent n'est pas exploitable.
...
pub fn event_indisponibilite(
    ...
    json!({
        ...
        "severity": 2,
        "message": format!("collector-mail {source} : {cause} ({raison}) — {detail}"),
        "dedup": dd,
        "fields": fields,
    })
}
...
mod tests {
    ...
    fn une_identite_illisible_ne_rend_aucun_nom() {
        ...
        // ② le cas nominal.
        std::fs::write(&f, "boite-01\n").unwrap();
        let v = identite_hote_depuis(&f, None);
        assert_eq!(v.verdict(), VERDICT_LU);
        assert_eq!(v.valeur().map(String::as_str), Some("boite-01"));
    ...
    /// et un aveu de ce collecteur doit reconnaitre les memes mots. Cette garde lit donc le module du
