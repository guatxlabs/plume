...
// Ce fichier est COPIÉ à l'identique dans chaque caisse qui possède ses temporaires (daemon, agent,
// collector-mail, collector-syslog) : la garde `check_tmp_possede_copies_are_identical.py` refuse toute
// divergence, et la CI arme le balayage « aucun temporaire laissé » sur chaque caisse qui le porte
// (`P8.9-o`). Hors du démon, `racine_systeme` n'a pas d'appelant de production : dit ici, pas caché.
#![allow(dead_code)]
