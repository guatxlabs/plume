...
fn phase_a_cold_route_count_by_source_equals_raw_zero_parquet() {
    ...
    let rr = crate::try_cold_rollup_route("search | stats count by source", from, to, None, b, i64::MAX)
...
fn phase_a_cold_route_multidim_by_dims_equals_raw_b2() {
    ...
    let rr = crate::try_cold_rollup_route(soql, from, to, None, b, i64::MAX).expect("route cold multi-dim (B2)");
    let low = rr.sql.to_lowercase();
...
fn phase_a_hot_cold_union_no_double_count_at_boundary() {
    ...
    let rr = crate::try_cold_rollup_route("search | stats count by source", 0, 0, None, b, i64::MAX).unwrap();
...
fn phase_a_dc_timechart_secondfilter_fall_back_no_cold_route() {
    ...
    assert!(crate::try_cold_rollup_route("search | stats dc(host) by source", 0, 0, None, b, i64::MAX).is_none(), "dc() by -> pas de route cold");
    assert!(crate::try_cold_rollup_route("search | stats dc(host)", 0, 0, None, b, i64::MAX).is_none(), "dc() -> pas de route cold");
    assert!(crate::try_cold_rollup_route("search | timechart count", 0, 0, None, b, i64::MAX).is_none(), "timechart -> pas de route cold");
    assert!(crate::try_cold_rollup_route("search source=web status=500 | stats count by path", 0, 0, None, b, i64::MAX).is_none(), "2e filtre non exprimable -> pas de route cold (angle mort évité)");
    assert!(crate::try_cold_rollup_route("search | stats avg(severity) by source", 0, 0, None, b, i64::MAX).is_none(), "avg() -> pas de route cold");
}
...
fn phase_a_generic_unknown_source_rolls_up_zero_config() {
    ...
    let rr = crate::try_cold_rollup_route("search | stats count by source", from, to, None, b, i64::MAX).unwrap();
    let res = crate::run_query_ex(&dbp, &rr.sql, 60_000, None).unwrap();
