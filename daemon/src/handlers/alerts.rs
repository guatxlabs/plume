...
pub(crate) fn alerts_query_page(conn: &Connection, f: &FiltreAlertes, group_col: Option<&str>, group_val: &str, limit: i64, offset: i64, want_total: bool) -> (Vec<Value>, Option<i64>, FinDeParcours) {
    ...
    let base = "SELECT alert.id,alert.ts,alert.rule,alert.severity,alert.title,alert.status,\
                ...
                COALESCE(alert.basis,''),COALESCE(alert.basis_ref,'') \
    ...
    let parcours = match stmt.query_map(binds.as_slice(), |r| {
        ...
        Ok(json!({
            ...
            // `P11.14-h` — SUR QUOI l'alerte est FONDÉE, tel que le démon l'a écrit à la levée (vocabulaire fermé
            // de `fondement.rs` ; vide = antérieure à v121, non déclaré) et, pour un instantané, son genre.
            "basis": r.get::<_, String>(17)?, "basis_ref": r.get::<_, String>(18)?,
