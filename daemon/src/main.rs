...
mod controles_de_defense; // P11.18-i : CE QU'UN INSTANTANÉ DE CONTRÔLES DIT
mod fondement; // P11.14-h : SUR QUOI UNE ALERTE EST FONDÉE, écrit à la levée dans un vocabulaire fermé
mod population_de_calibrage; // P4.12-g : SUR QUELLE POPULATION une règle livrée a été calibrée, lisible par le code — l'alerte NOMME les contrôles manquants, la machine et depuis quand (les trois étaient déjà en base, aucune n'était rendue) ; et un catalogue VIDE se dit au lieu de rendre « 0 manquant »
...
pub(crate) use handlers::panneau_avoue::{self, PanneauCompile}; // P10.5-i : le coffre de la compilation d'un panneau, de `panel_cache`, et de l'horizon avoué
...
fn main() {
    ...
    if args.get(1).map(String::as_str) == Some("backup") {
        ...
        // (défaut, aucun clair sur disque) ou une copie SQLite complète (chemin historique) — cf. backup/dump_restauration.rs.
