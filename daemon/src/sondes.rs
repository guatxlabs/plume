...
// TOUTE ligne ingérée : MESURÉ 25,5 o/ligne (5 537 792 o pour 217 280 lignes) soit ~250 Mio sur les
// 9,8 M lignes de la prod, plus une insertion btree sur le CHEMIN D'INGEST CHAUD. Le partiel n'indexe
// que les battements : MESURÉ 21,8 o/ligne de battement (376 832 o pour 17 280), soit ~1,5 Mio pour
// 8 collecteurs battant toutes les 5 min sur 30 j de rétention — 166x moins, et une insertion btree
// toutes les ~37 s au lieu d'une par event. C'est l'objection qui avait fait ÉCARTER le correctif en
// v104 (cf. `handlers/freshness.rs`, « amplification d'écriture sur le chemin d'ingest chaud ») :
// elle visait juste, elle ne visait que le composite plein.
//
...
// la cardinalité de la FLOTTE ; `event` compte ~9,8 M lignes en production. La portée est donc DÉCLARÉE
...
pub(crate) enum Cout {
    ...
    /// suit donc la FLOTTE, jamais les ~9,8 M lignes d'`event` (invariance vérifiée par mutation du
