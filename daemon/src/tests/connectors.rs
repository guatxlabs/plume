    ...
    fn dest_v92_migration_creates_table() {
        let conn = test_db();
        let v: String = conn.query_row("SELECT value FROM meta WHERE key='schema_version'", [], |r| r.get(0)).unwrap();
        assert_eq!(v, "111", "schema_version à la tête après migrate (#59 gouvernance legal_hold/ledger_sink)");
    ...
    fn v97_migration_creates_ko_reliquat_tables() {
        let conn = test_db();
        let v: String = conn.query_row("SELECT value FROM meta WHERE key='schema_version'", [], |r| r.get(0)).unwrap();
        assert_eq!(v, "111", "schema_version à la tête après migrate (#60 macros/auto-lookups/scheduled-reports/workflow-actions)");
        for (tbl, need) in [
