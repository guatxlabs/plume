    ...
    fn rollup_route_declines_dc_path_by_srcip_correlation_rule21() {
        assert!(try_rollup_route("search source=web status=404 | stats dc(path) by src_ip", 0, 0, None, RollupCoverage::asserted_by_the_test(i64::MAX, i64::MAX), DimRollupCoverage::all_asserted_by_the_test()).is_none(),
    ...
    fn rollup_route_still_routes_expressible_queries() {
        ...
        // les buckets de la fenêtre, y compris ceux que le job n'avait jamais agrégés — mesuré le 31/07 : `0`
        // servi contre 5 696 réels. On fige donc la forme AVEC sa borne de couverture, sur une bande explicite.
        let b = try_rollup_route(
            "search source=web | stats count by status", 0, 0, None,
            RollupCoverage::asserted_by_the_test(i64::MAX, i64::MAX),
            DimRollupCoverage::asserted_by_the_test(1_000_000, 2_000_000, 2_000_000, i64::MAX),
        )
        .expect("B doit router");
        assert_eq!(
            b.sql,
            "SELECT val AS \"status\", SUM(n) AS \"count\" FROM event_dim_rollup WHERE source='web' AND dim='status' \
    ...
    fn rollup_route_expressibility_parity_env_modes() {
        for env in [None, Some("staging")] {
            ...
            assert!(try_rollup_route("search source=web status>=500 | stats count by src_ip", 0, 0, env, RollupCoverage::asserted_by_the_test(i64::MAX, i64::MAX), DimRollupCoverage::all_asserted_by_the_test()).is_none(),
                    "décline stable quel que soit env={env:?}");
            // exprimable -> route dans les DEUX modes (env ajoute juste un env_id au WHERE).
            let r = try_rollup_route("search source=web | stats count by status", 0, 0, env, RollupCoverage::asserted_by_the_test(i64::MAX, i64::MAX), DimRollupCoverage::all_asserted_by_the_test()).expect("route stable");
    ...
    fn rollup_route_qry1_approx_and_recency_note() {
        ...
        let r = try_rollup_route_at("search | stats count by source", recent_from, now_ts, None, now_ts, RollupCoverage::asserted_by_the_test(i64::MAX, i64::MAX), DimRollupCoverage::all_asserted_by_the_test()).expect("route A récente");
