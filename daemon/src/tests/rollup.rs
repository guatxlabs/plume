    ...
    fn rollup_route_declines_status_by_srcip_correlation_rule22() {
        ...
        for q in [
            ...
            assert!(try_rollup_route(q, 0, 0, None, i64::MAX).is_none(), "rule22 corrélation status×src_ip DOIT décliner -> raw : {q}");
        }
    ...
    fn b2_multidim_parity_rollup_equals_raw() {
        ...
        for soql in [
            ...
            let rr = try_rollup_route(soql, 0, 0, None, i64::MAX)
    ...
    fn b2_multidim_declines_non_grain_dims() {
        ...
        for q in [
            ...
            assert!(try_rollup_route(q, 0, 0, None, i64::MAX).is_none(), "src_ip approx dans le group-by DOIT décliner -> raw : {q}");
        }
        ...
        for q in [
            ...
            assert!(try_rollup_route(q, 0, 0, None, i64::MAX).is_none(), "dim hors grain dans le multi-dim DOIT décliner -> raw : {q}");
        }
        ...
        for q in [
            ...
            assert!(try_rollup_route(q, 0, 0, None, i64::MAX).is_none(), "host/action (COALESCE '' -> divergence NULL/'') DOIT décliner -> raw : {q}");
        }
        // doublon -> refus (garde-fou).
        assert!(try_rollup_route("search | stats count by source,source", 0, 0, None, i64::MAX).is_none(), "doublon de dim DOIT décliner");
        ...
        let a = try_rollup_route("search | stats count by source", 0, 0, None, i64::MAX).expect("ROUTE A single-source intacte");
        assert!(a.sql.contains("GROUP BY source ORDER BY"), "ROUTE A single-dim inchangée (GROUP BY source seul) : {}", a.sql);
        assert!(try_rollup_route("search | stats count by severity", 0, 0, None, i64::MAX).is_none(), "single `by severity` inchangé (pré-B2) -> raw");
    }
    ...
    fn b2b_multidim_merge_bounds_exact_no_freshness_lag() {
        ...
        let past = try_rollup_route_at(soql, from, recent, None, now_ts, i64::MAX).unwrap();
        ...
        let recent_w = try_rollup_route_at(soql, from, now_ts, None, now_ts, i64::MAX).unwrap();
        ...
        assert!(
            try_rollup_route_at(soql, cur + 100, cur + 200, None, now_ts, i64::MAX).is_none(),
    ...
    fn b2b_multidim_merge_parity_multi_hour() {
        ...
        for &(from, to) in windows {
            let rr = try_rollup_route_at(soql, from, to, None, n, i64::MAX)
        ...
        let rr = try_rollup_route_at(sf, cur - 4 * 3600 + 55, n, None, n, i64::MAX).unwrap();
    ...
    fn b2b_multidim_recent_events_after_rollup_exact() {
        ...
        let soql = "search | stats count by source,severity";
        let rr = try_rollup_route_at(soql, cur - 2 * 3600, n, None, n, i64::MAX).unwrap();
    ...
    fn b2_multidim_killswitch_disables_route() {
        ...
        let multidim = try_rollup_route("search | stats count by source,severity", 0, 0, None, i64::MAX);
        let single = try_rollup_route("search | stats count by source", 0, 0, None, i64::MAX);
    ...
    fn b2_adverse_host_null_merged_with_empty_wrong_counts() {
        ...
        assert!(try_rollup_route(soql, 0, 0, None, i64::MAX).is_none(), "host hors grain -> DÉCLINE -> scan raw : {soql}");
    ...
    fn b2_adverse_action_missing_merged_with_empty_wrong_counts() {
        ...
        assert!(try_rollup_route(soql, 0, 0, None, i64::MAX).is_none(), "action hors grain -> DÉCLINE -> scan raw : {soql}");
    ...
    fn b2_multidim_bench_rollup_vs_rawscan() {
        ...
        let from = cur - 4 * 3600;
        let to = cur - 3600 - 1;
        let rr = try_rollup_route_at(soql, from, to, None, n, i64::MAX).unwrap();
