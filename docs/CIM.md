...
| `config`    | Auto-report du collecteur : transparence des filtres (**whitelists #10**) **et DISPONIBILITÉ** — `fields.collect_status` ∈ {`unavailable`, `disabled`, `fallback`} + `fields.reason` : les deux premiers disent qu'un capteur ne PEUT PAS collecter (dépendance/source/réglage absent), au lieu de sortir silencieusement en succès. Voir `plume_unavailable` / `plume_disabled` (`collectors/lib.sh`). Un septième mot de `reason` ne décrit PAS une incapacité : `collection-capped` dit que le capteur a collecté puis a COUPÉ lui-même, à une borne QUI EST SIENNE (plafond de lignes par passage, borne de durée sur une commande d'exploitant) — la source allait bien, la couverture est incomplète par construction, et le détail porte le nombre de lignes écartées (ou « nombre inconnu » quand la borne ne permet pas de le lire). Il est interdit sur les primitives qui SORTENT (une borne franchie n'arrête pas un passage) ; sa porte est `plume_collecte_tronquee`. **TROISIÈME ÉTAT, DÉCLARÉ le 2026-09-09 (`P4.6-c`)** : `fallback` / `reason=unreadable-config` dit qu'un RÉGLAGE n'a pas pu être lu et qu'un défaut s'applique à sa place, la collecte étant ENTIÈRE — mesuré le 2026-08-27 sur `collectors/custom.sh` (`MAX=deux` → 4 événements publiés), qui empruntait alors `unavailable / missing-config` et faisait basculer la pastille d'une source saine. Sa porte est `plume_reglage_illisible` (`collectors/lib.sh`), sévérité 1, interdite sur les primitives qui SORTENT ; la règle livrée `de-collector-unavailable` ne l'interroge pas, et il se requête : `search category=config collect_status=fallback`. **`fields.hors_vocabulaire`** (décision du 2026-09-10, `P11.19-b`) : quand un producteur Rust (agent, collecteur mail, collecteur syslog) émet pour `reason` ou `cause` un mot hors de l'ensemble qu'il déclare, le mot part TEL QUEL et ce champ nomme, séparés par des virgules, le(s) champ(s) concerné(s) — rien n'est perdu, la dette est visible et requêtable : `search category=config hors_vocabulaire=*`. | `web`, `nft`, `portscan`, `auditd`, **tout capteur incapable** |
