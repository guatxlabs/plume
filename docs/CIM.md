...
| `config`    | Auto-report du collecteur : transparence des filtres (**whitelists #10**) **et DISPONIBILITÉ** — `fields.collect_status` ∈ {`unavailable`, `disabled`} + `fields.reason` dit qu'un capteur ne PEUT PAS collecter (dépendance/source/réglage absent), au lieu de sortir silencieusement en succès. Voir `plume_unavailable` / `plume_disabled` (`collectors/lib.sh`). Un septième mot de `reason` ne décrit PAS une incapacité : `collection-capped` dit que le capteur a collecté puis a COUPÉ lui-même, à une borne QUI EST SIENNE (plafond de lignes par passage, borne de durée sur une commande d'exploitant) — la source allait bien, la couverture est incomplète par construction, et le détail porte le nombre de lignes écartées (ou « nombre inconnu » quand la borne ne permet pas de le lire). Il est interdit sur les primitives qui SORTENT (une borne franchie n'arrête pas un passage) ; sa porte est `plume_collecte_tronquee`. **BORNE DU CONTRAT, ÉCRITE PARCE QU'ELLE EST DÉJÀ FRANCHIE** (mesuré le 2026-08-27) : la phrase « `reason` dit qu'un capteur ne PEUT PAS collecter » vaut pour les MOTS, plus pour tous leurs EMPLOIS. `collectors/custom.sh` publie `unavailable / missing-config` quand une borne mal écrite retombe sur son défaut, ALORS QUE la source est intégralement collectée — la règle livrée `de-collector-unavailable` alerte et la pastille bascule pour une source saine. Le corriger demande un `collect_status` que ce document ne déclare pas ; tant qu'il n'est pas déclaré, l'écart est ici plutôt que sous-entendu. | `web`, `nft`, `portscan`, `auditd`, **tout capteur incapable** |
