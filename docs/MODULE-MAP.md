...
| `handlers/catalogue_attack.rs` | `GET /api/attack/catalogue` (`P11.6-c`): the ATT&CK catalogue served **whole** — every parent technique with its name and primary tactic, every named sub-technique with its composed name, the composition template for a sub-technique whose parent alone is known, and both counts. A constant object derived from `attack_names.rs`: no database read, no query permit. The console derives every technique label from it and carries no name table of its own. | Yes |
...
| `detection_aveugle.rs` | **A rule that cannot fire is an extinguished detection, and says so**: (`P3.9-a`) the cause of each abandon is kept (typed, not folded into `None`), and a rule abandoned tick after tick is surfaced as blind, not as calm; (`P9.5-a`) a rule whose query pins a `source` that **no shipped file emits and no shipped probe observes** is derived — never listed by name — so the seed can ship it dark instead of letting the ATT&CK matrix count its technique as covered — and because the seed only ever runs on a FRESH database, the same derivation is applied again at READ time (`lire_la_couverture_des_regles`, the single point every coverage surface goes through, which returns THREE states rather than two: a rule that is enabled but that nothing on this base can feed is counted apart, WITH the sources it is missing — never folded back into "nobody ever wrote a rule"), where a source this base has actually RECEIVED events from also counts as a producer: an already-deployed install stops claiming a technique covered without anything being switched off; and since 2026-09-10 a FOURTH state through the same door (`ENONCE_TAGS_ETEINTS`): the DISABLED rules that tag a technique, each with the sources nothing produces (empty = enabling it is enough; non-empty = wire first), so a fresh install no longer renders a rule seeded dark as "nobody ever wrote a rule" | Yes |
| `entrees_scriptees.rs` | **The gesture that wires a source, derived from what the repo ships (`P9.5-a`, 2026-09-10).** Two mirror tables held by witnesses — the shipped scripted inputs under `deploy/*.input.example` (source, file, destination read from the file's first line) and the host directory where bootstrap installs `collectors/*.sh` — and `geste_de_branchement(source)`: scripted input (file AND destination), then shipped shell collector (file AND bootstrap directory), then any shipped emitting file (file only), else `None` — so the ATT&CK matrix can NAME the file to copy and where, and say when the repo cannot wire a source instead of inventing a path | Yes |
| `fondement.rs` | **What an alert is FOUNDED on, written at raise time in a closed vocabulary (`P11.14-h`).** Four grounds (rule, snapshot, heartbeat, sensor) written into `alert.basis` by every raising site — a source-derived witness refuses a site that forgets it — so the console reads the ground instead of guessing it from the rule token; a pre-v121 alert keeps an empty ground and its honest refusal. | Yes |
| `population_de_calibrage.rs` | **On which population a shipped rule's threshold was calibrated, readable by code (`P4.12-g`).** One table (rule name → sources) written by the seeders on a fresh base and replayed by migration v122 on an existing one, never overwriting an operator's declaration; the rule loop writes at fire time the imputed sources OUTSIDE that population (`rule.population_vue`) — derived from the imputation already computed, no extra query — and the console shows them on the rule's row. | Yes |
...
| `vieillissement_serie.rs` | **What a cold aging pass cost (P10.5-a)** — one journal line *and* one series in `metric` per pass: days (candidate/aged/deferred/failed/skipped), rows written to Parquet, rows actually deleted from hot, files, on-disk bytes, duration, **thread** CPU, and a **window-scoped** RSS peak (`VmHWM` reset to the current RSS via `/proc/self/clear_refs`, then validated — the raw `VmHWM` is a since-boot maximum and would report an unrelated peak). A suspended pass publishes **no** work counters, only a named `..._ok{cause}=0`: a hole means "not measured", never zero. **Deliberately NOT feature-gated** (same reason as `cold_banniere.rs`): the publication logic and the measurement instrument must be testable in the DEFAULT build; only the caller (`cold_store/aging.rs`) is gated | Yes — pure functions + one bounded window per pass, and what that window costs is now **counted, not timed**: exactly four reads of `/proc/self/status` and one write to `clear_refs`, asserted in both directions (and on the value written) by `ouvrir_et_clore_une_fenetre_ne_fait_que_quatre_lectures_de_proc_et_une_ecriture`. The dated duration benchmark that stood in this cell until 2026-09-02 (`P11.23-c`) had outlived its witness: `P11.23-a` did not raise that ceiling, it **deleted it together with the grandeur it bounded**, because the ratio it compared accused a healthy composition on the build machine. A duration is still printed by the pass as a reference, never asserted — so pointing a reader at one sent them at something this repo had stopped guarding |
...
| `backup` | Incremental verbatim escrow of sealed day-files: the PLAN (pure, testable), played by `escrow_local` in the daemon and readable by `cold-backup-plan` for an external executor. Symmetric by design; daemon never deletes remote objects. |
| `escrow_local` | `P7.20-f` — the executor of that plan: copies each sealed day-file verbatim under `<backup destination>/cold/<tenant>/<env>/…` (the destination IS the memory: keys already there are the plan's `remote_keys`), atomic per file, never deletes, counts every failure and unreadable entry. Called after each native scheduler cycle and by `cold-escrow`. |
...
| `registres.js` | service | The leaf module that holds the registries several modules FILL at load time (technique doors, attached charges, the IOC search state). It imports nothing, so it is evaluated first whatever the entry door of the module graph: the value written at load time is never in a temporal dead zone (`P11.21-f`). It decides nothing — who fills and who reads stays where the gesture has a meaning. | `attack.js` (technique doors), `navigation.js` (attached charges), `threatintel.js` (search state) |
| `catalogue_attack.js` | service | The leaf module that derives every ATT&CK technique label from the catalogue the daemon serves (`GET /api/attack/catalogue`, `P11.6-c`): a registry filled once per session, a resolver that mirrors the daemon's rule (served name, or a sub-technique composed from the served parent name and the served template), and for every name it cannot give an **explicit state and reason** — never an empty string. It imports nothing: the network gesture is lent to it by `login.js` at session opening, and the harness poses a served object without network. The hand-written 14-label table it replaces is gone. | `core.js` (group header), `alerts.js`, `detection_admin.js`, `login.js` |
| `coupe_de_liste.js` | service | The single reading of the **cut confession** the daemon serves next to a bounded list (`served`/`window`/`total`/`total_capped`/`truncated`, bare or prefixed by the list's name, `P11.22-g`): one function decides whether a bound bit, one phrase says it, nothing is said when nothing is cut, and an absent confession is reported as *unknown* — never rendered as zero. Also says the cut the console makes itself (twelve queue chips) and a metrics sample cut. | `cases.js`, `risk.js`, `datamodels.js` |
...
| `plage_de_dates.js` | service | The console's date-range picker: calendar-day and instant parsing to epoch at the analyst's local time (a non-existent day like 2026-02-31 is refused, not silently rolled over), the grain table, the range reader, and the control and modal that pose a window on a target. A pure move out of `core.js` (`P7.18-a`, modularity): the block has no back-dependency into the rest of `core.js` (it reads only `LANG`), so it leaves the shared surface that nine open lots had to queue behind. | `audit.js`, `dataaccess.js`, `app.js` |
...
| `representations.js` | service | The single list of the nine result representations (table, bars, line, stat, gauge, pie, donut, heatmap, histogram), their icon, and the gesture that poses them in a `<select>` (`P11.20-d`). Measured 2026-08-25: the search bar offered four of nine because the list was written four times (two arrays and an HTML string in `dashboards.js`, a static `<select>` in `index.html`) and an addition never reached the editor. It imports nothing. | `app.js` (Explore `#viz`), `dashboards.js` (panel switcher, panel form, add-panel form) |
