...
| `backup/` | Compressed+encrypted backup `age(zstd(charge))`, split into one façade and two pure submodules (below); callers keep the `crate::backup::X` paths through `pub(crate)` re-exports | Guarded — scrypt lockstep with cold crypto |
| `backup/mod.rs` | Façade: the envelope — charge is a streamed typed dump by default (no plaintext file on disk), or a full SQLite copy on the legacy path; symmetric (SQLCipher key) or asymmetric recipient; restore detects both by header marker; fixed scrypt work factor; single configuration path for backup settings (`cfg()`, never bare `env::var`); plaintext-temp guard and orphan sweep | Guarded — scrypt lockstep with cold crypto |
