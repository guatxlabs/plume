...
| **P7.8-a** | Observabilité des requêtes | ⬜ | Dix-neuf routes consomment le sémaphore interactif — dont l'export — sans aucune métrique de latence. Seul l'instrument a été revalidé ; le décompte date et n'a pas été refait. |
...
| **P8.3-a** | Restauration | ⬜ | Le test de restauration prouve l'existence, la bonne forme et le schéma de chiffrement de l'archive, jamais la restauration : c'est délibéré, aucune clé privée de séquestre ne vit à côté des sauvegardes. Ce qui manque est la preuve qu'un exercice de restauration hors ligne a bien lieu. |
...
| **P8.15-a** | Historique public | ✅ | L'historique et les documents publiés s'adressaient à un lecteur interne et portaient une identité personnelle. Messages réécrits à la voix impersonnelle en conservant mesure, réfutation et clé ; identité de projet posée ; deux gardes versionnées refusent une identité non canonique et les familles objectives de fuite. Le **style** n'est pas mécanisé — une garde qui prétendrait juger le ton ferait du bruit et finirait désarmée. |
...
| **P8.17-a** | Avis de securite des dependances | 🔵 | Deux avis remontes par l'audit de dependances, aucun lie a une modification du depot. h2 0.4.15 (RUSTSEC-2026-0258, avis du 2026-08-17) se corrige par un bump de patch vers 0.4.16. rkyv 0.7.46 (RUSTSEC-2026-0235) demande un SAUT MAJEUR vers 0.8.17, probablement via le parent du tier froid colonnaire : l'atteignabilite du correctif depend de ce parent, et se mesure avant d'agir. |
| **P8.18-a** | Outil d'audit trop ancien pour sa base | ⬜ | Sur la bibliotheque, l'audit de dependances n'atteint meme pas l'etape de scan : la base d'avis contient une entree en CVSS 4.0 que la version installee ne sait pas lire, et l'outil s'arrete sur une erreur d'analyse. Ce n'est ni une vulnerabilite ni une regression du code — c'est un outil non epingle qui derive de sa base. Correctif : fixer une version recente dans le workflow, et NOMMER la version, faute de quoi le meme decalage reviendra sans prevenir. |
...
| S6 | Surveillance d'intégrité | ⬜ | Le durcissement du service pose une protection du répertoire personnel qui écrase celle, plus permissive, dont la surveillance d'intégrité a besoin : les clés SSH autorisées — un vecteur de persistance que le collecteur annonce lui-même surveiller — ne le sont donc pas. Relâcher le bac à sable est un arbitrage qui appartient à l'exploitant. |
...
charge d'analystes. Dix-neuf routes le consomment sans métrique de latence (P7.8-a).
...
perte. Le **test** de restauration, lui, ne restaure pas (P8.3-a) : il vérifie l'existence, la bonne
forme et le schéma de chiffrement, aucune clé privée de séquestre n'étant placée à côté des
sauvegardes. Un exercice de restauration réel reste un geste d'exploitant.
