...
const I18N_EN = {
  ...
  "Enregistrer la requête comme panneau": "Save query as a panel",
  ...
  "ce compte se connectera de nouveau avec le seul mot de passe ; les codes de secours émis deviennent caducs.": "this account will sign in again with the password alone; the recovery codes issued become void.",
  "destruction cryptographique IRRÉVERSIBLE : la clé est oubliée et la base chiffrée supprimée, aucune restauration possible.": "IRREVERSIBLE cryptographic destruction: the key is forgotten and the encrypted database deleted, no restoration is possible.",
  "cet utilisateur ne pourra plus ouvrir ce tenant ni y lire quoi que ce soit dès sa prochaine requête.": "this user will no longer be able to open this tenant or read anything in it from their next request.",
  "les événements que cette règle filtrait, masquait ou routait seront de nouveau ingérés tels quels dès le prochain lot ; la règle ne se restaure pas.": "the events this rule filtered, masked or routed will be ingested as-is again from the next batch; the rule cannot be restored.",
  ...
  "Intégrité (FIM)": "Integrity (FIM)", "Jetons": "Tokens", "Lookups": "Lookups", "Modèles & Pivot": "Models & Pivot",
  ...
  "RBAC Kubernetes (kube-rbac)": "Kubernetes RBAC (kube-rbac)", "Risque": "Risk", "Rétention": "Retention",
  ...
  "Copier": "Copy", "Absolu — intervalle précis (figé)": "Absolute — exact interval (frozen)",
  ...
  "Début": "Start", "Fin": "End", "Le début doit précéder la fin.": "The start must precede the end.",
  ...
  "Ce qu'on attend": "What is expected",
  "Motif (requis pour éteindre l'alerte)": "Reason (required to silence the alert)",
  "déclarer": "declare",
  "déclaration enregistrée": "declaration saved",
  "déclaration retirée": "declaration removed",
  "un signal est attendu — son silence est un incident": "a signal is expected — its silence is an incident",
  "son silence est attendu — machine de test, banc, saisonnière": "its silence is expected — test box, bench, seasonal",
  "retirée du parc — décommissionnée": "removed from the estate — decommissioned",
  "ex : banc de test, machine décommissionnée le …": "e.g. test bench, machine decommissioned on …",
  "Actions → « déclarer » : silence attendu (elle reste au parc, sans alerter) ou retirée du parc (persistant, réversible, audité).": "Actions → “declare”: silence expected (it stays in the estate, without alerting) or removed from the estate (persistent, reversible, audited).",
  "La déclaration demande le rôle éditeur ou administrateur (geste persistant, réversible, audité).": "Declaring requires the editor or administrator role (persistent, reversible, audited action).",
  "Dire ce qu'on attend de cette machine : un signal, un silence normal, ou la sortie du parc (persistant, réversible, audité)": "State what is expected of this machine: a signal, a normal silence, or leaving the estate (persistent, reversible, audited)",
  "Déclarée : quelqu'un a dit ce qu'on attend de cette machine. Le détail dit qui, quand, et pourquoi.": "Declared: someone stated what is expected of this machine. The detail says who, when, and why.",
  "Personne ne l'a déclarée — ni un enrôlement, ni un humain de cette installation. Son silence alerte donc, ce qui est le défaut sûr : on ne peut pas deviner qu'une machine a été décommissionnée.": "Nobody declared it — neither an enrolment nor a human of this installation. Its silence therefore alerts, which is the safe default: a decommissioned machine cannot be guessed.",
  "Retirer la déclaration : la machine reprend le défaut — son silence alerte de nouveau": "Remove the declaration: the machine returns to the default — its silence alerts again",
  "Retirée du parc par un humain de cette installation : elle reste listée (son historique n'est pas effacé) mais sort du dénominateur et de l'alerte.": "Removed from the estate by a human of this installation: it stays listed (its history is not erased) but leaves the denominator and the alert.",
  "Les parts font le tout : frais + en retard + muets + muets attendus = le parc. Seuls les « muets » alertent — un silence déclaré attendu, non. Les machines retirées sont hors du parc et comptées à part.": "The shares make the whole: fresh + late + silent + expected-silent = the estate. Only “silent” alerts — a declared expected silence does not. Removed machines are outside the estate and counted separately.",
  "Un motif est requis : éteindre l'alerte sur une machine sans dire pourquoi est illisible six mois plus tard.": "A reason is required: silencing the alert on a machine without saying why is unreadable six months later.",
  "Une ligne par hôte/machine (endpoint où un agent pousse) — statut de l'agent, dernier signal, enrôlement, et ce qu'on ATTEND de la machine. Un hôte « muet » n'est un incident que si personne n'a déclaré le contraire : une machine de test ou décommissionnée se déclare, et cesse alors d'alerter. Affichage seul — aucune commande d'hôte depuis la console (version/OS de l'agent non transmis par le collecteur, non affichés). → Pour les sources par type de donnée, voir Inventaire des sources.": "One row per host/machine (endpoint where an agent pushes) — agent status, last signal, enrolment, and what is EXPECTED of the machine. A “silent” host is an incident only if nobody declared otherwise: a test or decommissioned machine is declared, and then stops alerting. Display only — no host command from the console (agent version/OS are not transmitted by the collector, not displayed). → For sources by data type, see Source inventory.",
  "Statut = fraîcheur du dernier signal de l'hôte : frais (<15 min) · en retard (15 min–1 h) · muet (>1 h). « Attendu de l'hôte » dit ce que quelqu'un a déclaré de cette machine, et qui : un enrôlement (un jeton d'agent y est lié), l'exploitant (avec sa date et son motif), ou personne. Un silence n'est un incident QUE si personne n'a déclaré le contraire — une machine de test se déclare « silence attendu » (elle reste au parc, sans alerter), une machine décommissionnée se déclare « retirée » (elle sort du dénominateur, sans disparaître de la liste). Ces déclarations ne touchent ni la collecte, ni les règles, ni la rétention. « Signaux » = volume total reçu (rétention). « Enrôlement » = token d'agent lié à l'hôte ; « Dernier push agent » = dernier appel authentifié du token (mode mono-tenant). Version et OS de l'agent ne sont pas transmis par le collecteur (différés).": "Status = freshness of the host's last signal: fresh (<15 min) · late (15 min–1 h) · silent (>1 h). “Expected from host” says what someone declared of this machine, and who: an enrolment (an agent token is bound to it), the operator (with their date and reason), or nobody. A silence is an incident ONLY if nobody declared otherwise — a test machine is declared “silence expected” (it stays in the estate, without alerting), a decommissioned machine is declared “removed” (it leaves the denominator, without disappearing from the list). These declarations touch neither collection, nor rules, nor retention. “Signals” = total volume received (retention). “Enrolment” = agent token bound to the host; “Last agent push” = last authenticated call of the token (single-tenant mode). Agent version and OS are not transmitted by the collector (deferred).",
  ...
  "Libellé": "Label", "Réinitialiser": "Reset",
  ...
  "Tableaux de bord NON LUS : le démon a refusé et en nomme la cause —": "Dashboards NOT READ: the daemon declined and names the cause —",
  "Corrélations NON LUES : le démon a refusé et en nomme la cause —": "Correlations NOT READ: the daemon declined and names the cause —",
  "Indicateurs de compromission NON LUS : le démon a refusé et en nomme la cause —": "Indicators of compromise NOT READ: the daemon declined and names the cause —",
  "File d'actions NON LUE : le démon a refusé et en nomme la cause —": "Action queue NOT READ: the daemon declined and names the cause —",
  "Mode de réponse NON LU : le démon a refusé et en nomme la cause —": "Response mode NOT READ: the daemon declined and names the cause —",
  ...
  // `P10.20-g` (2026-09-16) — LES TROIS PHRASES DE `fleet.js`, ET ELLES SONT ICI PARCE QUE CE MODULE-LÀ
  // NE CONNAÎT PAS `LANG` : il rend des NŒUDS et écrit ses libellés en français, le lexique les traduit
  // — c'est sa forme depuis `P11.8-a`, et l'y changer sortirait ses quarante-deux voisins du regard de
  // la garde. Les deux premières sont des aveux à DEUX NŒUDS (la phrase seule, puis la cause SERVIE) ;
  // la troisième est la phrase ENTIÈRE du cas où le démon n'a servi AUCUNE cause — une version
  // antérieure à cette clé —, et elle dit alors elle-même ce que `pipeline_fresh: null` n'établit pas.
  "Enrichissement de la flotte NON LU : les hôtes ci-dessous ONT été lus et la liste est complète — ce qui manque est ce qu'on sait D'EUX. Les colonnes « Enrôlement » et « Attendu de l'hôte » y portent alors un repli, pas une observation. Le démon en nomme la cause —": "Fleet enrichment NOT READ: the hosts below WERE read and the list is complete — what is missing is what is known ABOUT them. The “Enrolment” and “Expected of the host” columns then carry a fallback, not an observation. The daemon names the cause —",
  "Santé du pipeline NON LUE : le démon ne l'a pas lue et en nomme la cause —": "Pipeline health NOT READ: the daemon did not read it and names the cause —",
  "Santé du pipeline NON LUE : le démon ne l'a pas lue, et cette réponse n'en nomme pas la cause. Ce n'est PAS « ingestion en panne » — rien n'a été observé, ni panne ni bonne santé ; l'inventaire d'hôtes ci-dessous, lui, n'est pas concerné par cette lecture.": "Pipeline health NOT READ: the daemon did not read it, and this response names no cause. This is NOT “ingestion down” — nothing was observed, neither failure nor good health; the host inventory below is not concerned by that read.",
